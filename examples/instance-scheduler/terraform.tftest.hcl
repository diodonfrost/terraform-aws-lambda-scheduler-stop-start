run "create_test_infrastructure" {
  ...
  assert {
    condition     = module.test_execution[0].instance_1_scheduled_state == "stopped" || module.test_execution[0].instance_1_scheduled_state == "stopping"
    error_message = "Virtual machine 1 to stop is not stopped"
  }

  assert {
    condition     = module.test_execution[0].instance_2_scheduled_state == "stopped" || module.test_execution[0].instance_2_scheduled_state == "stopping"
    error_message = "Virtual machine 2 to stop is not stopped"
  }

  assert {
    condition     = module.test_execution[0].instance_3_scheduled_state == "stopped" || module.test_execution[0].instance_3_scheduled_state == "stopping"
