run "create_test_infrastructure" {
  ...
  assert {
    condition = module.test_execution[0].asg_scheduled_suspended_processes == toset([
      "AZRebalance",
      "AddToLoadBalancer",
      "AlarmNotification",
      "HealthCheck",
  ...
  assert {
    condition     = length(module.test_execution[0].asg_not_scheduled_suspended_processes) == 0
    error_message = "Autoscaling group instances should not be suspended"
  }

  assert {
    condition     = module.test_execution[0].asg_instance_scheduled_state == "stopped" || module.test_execution[0].asg_instance_scheduled_state == "stopping"
    error_message = "Autoscaling group instance should be stopped"
  }

  assert {
    condition     = module.test_execution[0].asg_instance_not_scheduled_state == "running"
